...
setup_helm() {
  ...
  helm version $tls_enabled_arg

  helm_setup_purge_all=$(jq -r '.source.helm_setup_purge_all // "false"' <$1)
  if [ "$helm_setup_purge_all" = "true" ]; then
    local release
    for release in $(helm ls -aq)
    do
      helm delete $tls_enabled_arg --purge "$release"
    done
  fi
}
...
wait_for_service_up() {
  ...
  RESULT=`kubectl get endpoints $SERVICE -o jsonpath={.subsets[].addresses[].targetRef.name} 2> /dev/null || true`
  if [ -z "$RESULT" ]; then
    sleep 1
